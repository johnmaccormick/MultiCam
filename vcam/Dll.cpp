...
const AMOVIESETUP_FILTER AMSFilterVCam =
{
    ...
    L"Virtual Cam (use before/after MultiCam)",     // String name
...
{
    {
        L"Virtual Cam (use before/after MultiCam)",
        &CLSID_VirtualCam,
...
STDAPI RegisterFilters( BOOL bRegister )
{
    ...
    if(bRegister)
    {
        hr = AMovieSetupRegisterServer(CLSID_VirtualCam, L"Virtual Cam (use before/after MultiCam)", achFileName, L"Both", L"InprocServer32");
    }
    ...
    if( SUCCEEDED(hr) )
    {
        ...
        if( SUCCEEDED(hr) )
        {
            if(bRegister)
            {
                ...
                hr = fm->RegisterFilter(CLSID_VirtualCam, L"Virtual Cam (use before/after MultiCam)", &pMoniker, &CLSID_VideoInputDeviceCategory, NULL, &rf2);
            }
            else
            {
